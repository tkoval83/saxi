...
case class Text private (
  ...
  def render(): Paths = {
    ...
    processContent(effectiveFont, scale).zipWithIndex
      .foldLeft(Paths.empty) {
        case (paths, (line, idx)) =>
  ...
  private def splitLongWord(word: String, maxWidth: Double, font: Font): (String, String) = {
    ...
    def loop(
      idx: Int,
      currentWidth: Double,
      hasPreviousChar: Boolean
    ): (String, String) =
      if (idx >= word.length) (word, "")
      else {
        val char = word(idx)
        val charWidth = font.glyphs
          .get(char.toString)
          .map(_.advanceWidth)
          .getOrElse(0.0)

        val tracking = if (hasPreviousChar) modifiers.tracking else 0.0
        val totalWidth = currentWidth + tracking + charWidth

        if (totalWidth > maxWidth && idx > 0) {
          (s"${word.substring(0, idx)}-", word.substring(idx))
        } else {
          loop(idx + 1, totalWidth, hasPreviousChar = true)
        }
      }

    loop(0, 0.0, hasPreviousChar = false)
  }
  ...
  private def renderLine(line: String, font: Font, scale: Double): Paths = {
    if (line.isEmpty) return Paths.empty
    line
      .foldLeft((Paths.empty, 0.0, false)) {
        case ((paths, x, hasPrevious), char) =>
          font.glyphs.get(char.toString).fold((paths, x, hasPrevious)) { glyph =>
            val trackingOffset = if (hasPrevious) modifiers.tracking * scale else 0.0
            val newX = x + trackingOffset
            val charPath = glyph.paths
              .scale(scale)
              .translate(newX, 0)
            val advance = glyph.advanceWidth * scale
            (paths.combine(charPath), newX + advance, true)
          }
      }
      ._1
  }
  ...
  private def measureLine(line: String, font: Font): Double = {
    ...
    val advances = line.map { c =>
      font.glyphs.get(c.toString).fold(0.0)(_.advanceWidth)
    }.sum

    val totalTracking = modifiers.tracking * (charCount - 1)
    advances + totalTracking
  }
